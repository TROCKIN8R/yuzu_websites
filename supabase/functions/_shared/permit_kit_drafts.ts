/**
 * Save / resume helpers for IRCC permit-kit drafts (study, work, future visas).
 *
 * - Resume code + family name are stored as peppered SHA-256 hashes only.
 * - Form payload is AES-256-GCM encrypted (STUDY_KIT_DRAFT_ENCRYPTION_KEY).
 * - Table access is service-role only; Edge Function decrypts on resume.
 *
 * Crypto secrets stay STUDY_KIT_* for backward compatibility across kits.
 */
...
export type DraftsConfig = {
  /** Supabase table name, e.g. study_permit_drafts */
  table: string;
  /** Hint when table is missing (shown to operators via API error). */
  setupSqlHint?: string;
  /** Hint when family_name_hash column is missing. */
  familyNameSqlHint?: string;
};
...
export type DraftStore = {
  normalizeFamilyName: typeof normalizeFamilyName;
  normalizeResumeCode: typeof normalizeResumeCode;
  generateResumeCode: typeof generateResumeCode;
  formatResumeCode: typeof formatResumeCode;
  hashResumeSecrets: typeof hashResumeSecrets;
  saveDraft: (
...
/** Create a draft store bound to a specific drafts table. */
export function createDraftStore(config: DraftsConfig): DraftStore {
  const setupHint = config.setupSqlHint ||
    "Draft storage is not set up yet. Run the kit drafts SQL script in the Supabase SQL Editor.";
  const familyHint = config.familyNameSqlHint ||
    "Draft storage needs a schema update (family_name_hash column).";
  ...
  async function saveDraft(
    ...
    if (!familyName || familyName.length < 2) {
      return {
        ok: false,
        error:
          "Enter your family name (last name) before saving — it’s needed to resume later.",
      };
    }
    ...
    try {
      ...
      const { captchaToken: _c, consent: _consent, delivery: _d, ...safePayload } =
        input.payload;
    ...
    if (error) {
      ...
      if (msg.includes("family_name_hash") || msg.includes("column")) {
        return { ok: false, error: familyHint };
      }
  ...
  async function loadDraft(
    ...
    if (!familyName || familyName.length < 2) {
      return {
        ok: false,
        error: "Enter the family name (last name) used when you saved.",
      };
    }
    const code = normalizeResumeCode(input.code);
    if (code.length < 8) {
      return {
        ok: false,
        error: "Enter your resume code (for example YUZU-XXXX-XXXX).",
      };
    }
    ...
    if (!data) {
      return {
        ok: false,
        error:
          "No matching draft found. Check the code and family name — or the code may have expired (30 days).",
      };
    }
    ...
    try {
      ...
      return {
        ok: false,
        error:
          "Could not unlock this draft. It may have been saved before encryption was enabled — please start a new save.",
      };
    }
