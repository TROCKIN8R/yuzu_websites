(function initContractRouterDemo() {
    const root = document.getElementById('automationLab');
    const data = window.ContractRouterData;
    const table = window.ContractRouterTable;
    if (!root || !data || !table) return;

    const tableId = 'home-contracts-preview';
    const form = document.getElementById('homeContractForm');
    const formStatus = document.getElementById('homeContractFormStatus');
    const submitBtn = document.getElementById('home-contract-submit');
    const turnstileMount = document.getElementById('home-contract-turnstile');
    const contractPanel = document.getElementById('automation-panel-contracts');
    const nameInput = document.getElementById('home-contract-name');
    const emailInput = document.getElementById('home-contract-email');
    const consentInput = document.getElementById('home-contract-consent');
    ...
    table.init({
        id: tableId,
        tbodyId: 'homeContractTableBody',
        statusId: 'homeContractTableStatus',
        limit: 5,
        idleMs: 30000
    });
    ...
    function isContractPanelVisible() {
        return Boolean(contractPanel && !contractPanel.hidden);
    }
    ...
    function renderTurnstile() {
        ...
        if (!siteKey || !turnstileMount || !isContractPanelVisible()) return;

        destroyTurnstile();

        loadTurnstileScript()
            .then(() => {
                if (!window.turnstile || !isContractPanelVisible()) return;
    ...
    async function handleSubmit(event) {
        ...
        const source = 'yuzu.solutions/home-contract-router-demo';
    ...
    document.addEventListener('automation-panel-open', (event) => {
        if (event.detail?.tabId === 'contracts' && captchaRequired) {
