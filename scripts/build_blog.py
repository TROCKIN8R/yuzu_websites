...
<body class="min-h-screen flex flex-col nav-on-blog">
...
<main class="max-w-7xl mx-auto px-6 py-12 w-full">
    <a href="index.html" class="text-yellow-700 font-semibold text-sm hover:underline">{ui['back_blog']}</a>
    <article class="blog-article mt-6">
        <header class="blog-article-header">
            <span class="blog-category">{html.escape(category)}</span>
...
def render_index_page(articles: list[dict], locale: str) -> str:
    ...
    for article in articles:
        ...
        cards.append(
            f"""        <article class="blog-card yuzu-card">
            <span class="blog-category">{html.escape(category)}</span>
